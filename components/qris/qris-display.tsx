...
export function QrisDisplay({ payload, size = 240 }: QrisDisplayProps) {
  ...
  if (result.payload !== payload || !result.dataUrl) {
    return <div className="grid h-60 place-items-center rounded-xl bg-slate-100 text-sm text-slate-500">Membuat QR...</div>
  }

  return (
    <div className="grid place-items-center rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
